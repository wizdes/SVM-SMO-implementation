...
int examineExample(int i1)
{
	...
	r1 = y1 * E1;
	// checks to see if it violates the KKT conditions by more than the tolerance
	// E1 is the error associated with i1, E2 is the error associated with i2
		...
		|| (r1 > tolerance && alph1 > 0))
	{
...

		{
			...
			// try to get argmax of E1 - E2 (needs to be greater than 0)
			// what is E1/E2? It's the error for i1 and i2, respectively
			// abs(E1 - E2) 
			/// why is this important?
			// if this is valid, then apply the takeStep function
			// there are three ways to pick up the two alphas to resolve
		...
		}
		{
			...
			// now, try to iterate through the non-bound examples (things that have alpha)
			// this means it'll go through all the examples that have a non zero alpha
			// if this goes to takeStep, then we have to investigate if this requires a change
			// takeStep 
		...
		}
		{
			...
			// this just goes through all the training examples
...
// take step calculates alpha 1 and alpha 2 for i1 and i2
// if nothing happens, then it just returns 
int takeStep(int i1, int i2) {
	int y1, y2, s;
	...
	// get the two existing alphas, and the current error margins.
	alph1 = alph[i1];
	y1 = target[i1];
	if (alph1 > 0 && alph1 < C)
		E1 = error_cache[i1];
	...
	// calculates gamma, L and H (low and high bounds?)
	if (y1 == y2) {
	...
	// if they're equal, what does this mean?
	if (L == H)
		return 0;


	// 'ETA' is calculated here. What is that?
	...
	// if the delta bounding condition is too small, then no change!
	...
	// the rest are just updates to W/B/error cache
	// b is calculated here from alpha.
	{
		float b1, b2, bnew;
	...
	// the weights are calculated here.
	if (is_linear_kernel) {
	...
	// the error cache is updated here. this is to use later.
	{
		float t1 = y1 * (a1 - alph1);
		float t2 = y2 * (a2 - alph2);
...
int smomain(int argc, char *argv[]) {
	...
	// this populates all the static values that exist
	{
		int n;
		if (N > 0) {
			target.reserve(N);
	...
	// this is the main function that does the SMO calculation
	numChanged = 0;
	examineAll = 1;
	// the only case the loop breaks is if numChanged == 0, and examineAll == 0
	// this can only be the case if there are iterations where numChanged == 0 twice, the second one
	// required to be where examineAll == 1
	// if no /alpha is changed, then exit
